...
var app = {
  ...
  fixUp: function (jsonBlob, jsonQuery, transformFunction) {
         // for keys, will replace the values with transformation function 
    ...
    for(var k in queryDays) { 
        ...
        if(parseInt(dday)>=(new Date()).getDate()) {
          if(currentDay) { 
			...
			this.generateDivs();
            $('.innerInnerCell').click(function () { callCalendar() });
